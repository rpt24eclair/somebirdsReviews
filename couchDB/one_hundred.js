...
let names = ['Hilda', 'Wolfe','Guy', 'Abbott','Jacqueline', 'Cole', 'Otis', 'Jones','Gabriel', 'Weber','Ernesto', 'Ross'];
  ...
  material.forEach(material => {
    action.forEach(action => {
      ...
      curReviews.forEach((review, index) => {
        ...
        count += 1;
        reviewBulkData.push({ name: names[index], headline: headlines[index], review: review, rating: tempRating, fit_feedback: tempFitFeedback, shoe_id: model });
      });
...
module.exports = {
  shoeBulkData: shoeBulkData,
  reviewBulkData: reviewBulkData
};